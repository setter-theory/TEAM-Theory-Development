...
function welcomeView(){
 return `<main class="onboarding alia-home">
   <div class="alia-hero-v396">
     ...
     <img class="alia-character alia-character-v396" src="./icons/alia-standalone.png?v=0.54.2" alt="Alia">
   </div>
   ${savedTeamsView()}
   ...
   <div class="welcome-version">Version 0.54.2</div>
 </main>`;
}
...
function createTeamView(){
 return `<main class="onboarding compact form-onboarding create-team-screen">
   ...
   <section class="create-team-card">
     ...
     <div class="create-alia-zone"><div class="create-alia-bubble">チーム名は<br>後から変更できるよ♪</div><img src="./icons/alia-standalone.png?v=0.54.2" class="create-alia" alt="Alia"></div>
   </section>
...
function joinTeamView(){
 return `<main class="onboarding compact form-onboarding join-team-screen">
   ...
   <section class="join-team-card">
     ...
     <div class="join-alia-zone"><div class="join-alia-bubble">招待コードは<br>大文字・小文字を<br>気にしなくて<br>大丈夫だよ♪</div><img src="./icons/alia-standalone.png?v=0.54.2" class="join-alia" alt="Alia"></div>
   </section>
...
function renderDirectorTargetValue(){
 ...
 if(type==='grade') wrap.innerHTML=`<label class="settings-field"><span>対象学年</span><select id="directorTargetValue" class="input">${optionList(['1年','2年','3年'],'1年')}</select></label>`;
 else if(type==='position') wrap.innerHTML=`<label class="settings-field"><span>対象ポジション</span><select id="directorTargetValue" class="input">${optionList(POSITIONS.filter(x=>x!=='未設定'),'セッター')}</select></label>`;
 else wrap.innerHTML='';
}
...
function menuView(){
 ...
 return `<section class="menu-page menu-hub-page">
   <div class="menu-page-head menu-hub-head"><div><small>TEAM MENU</small><h2>メニュー</h2><p>${esc(a.teamName)}の情報・設定を選びます。</p></div><img src="./icons/alia-standalone.png?v=0.54.2" alt="Alia"></div>
...
function appInfoView(){
 ...
 ${settingsCard('TEAM Theory','教わるから、考えるへ。',`<div class="app-info-version"><small>VERSION</small><b>0.54.2</b></div><p class="app-info-copy">選手の意見を主役に、チームの話し合いと成長を支えるアプリです。</p><div class="cloud-foundation-status"><b>学校アカウント基盤</b><span>${cloudConfigured()?'クラウド接続済み':'Supabaseキー設定待ち'}</span></div>`)}
...
function exportTeamData(){
 const data={version:'0.54.2',exportedAt:new Date().toISOString(),localStorage:{}};
...
function resume(id){state.currentMeetingId=id;const m=getCurrent();state.selectedType=TYPES[m?.type]?m.type:null;state.view=m?.status==='closed'?'summary':'room';render()}
...
if ('serviceWorker' in navigator) {
  ...
  navigator.serviceWorker.register('./sw.js?v=0.54.2', { updateViaCache: 'none' })
    .then(reg => {
      reg.update().catch(()=>{});
